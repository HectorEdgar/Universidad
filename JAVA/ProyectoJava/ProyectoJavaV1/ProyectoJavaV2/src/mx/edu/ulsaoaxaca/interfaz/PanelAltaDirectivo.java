...
import mx.edu.ulsaoaxaca.Dao.DirectivoDao;
import mx.edu.ulsaoaxaca.Dao.DirectivoDaoImpl;
import mx.edu.ulsaoaxaca.Dao.EjecutivoDao;
import mx.edu.ulsaoaxaca.Dao.EjecutivoDaoImpl;
...
public class PanelAltaDirectivo extends JPanel {
	...
	public PanelAltaDirectivo() {
		...
		int y = 50;
		int x = 20;
		this.jlTituloRegistro = new JLabel("Registro de ejecutivos: ");
		...
		this.jlAccion = new JLabel("Accion: ");
		this.jlAccion.setBounds(x, y + 180, 250, 20);
		this.jtAccion = new JTextField();
		this.jtAccion.setBounds(x + 300, y + 180, 200, 20);

		this.jlDomicilioParticular = new JLabel("Domicilio Particular: ");
		this.jlDomicilioParticular.setBounds(x, y + 230, 250, 20);

		this.jlDomicilioParticularCalle = new JLabel("Calle : ");
		this.jlDomicilioParticularCalle.setBounds(x, y + 260, 250, 20);
		this.jtDomicilioParticularCalle = new JTextField();
		this.jtDomicilioParticularCalle.setBounds(x + 300, y + 260, 200, 20);

		this.jlDomicilioParticularColonia = new JLabel("Colonia : ");
		this.jlDomicilioParticularColonia.setBounds(x, y + 290, 250, 20);
		this.jtDomicilioParticularColonia = new JTextField();
		this.jtDomicilioParticularColonia.setBounds(x + 300, y + 290, 200, 20);

		this.jlDomicilioParticularEntidadFederativa = new JLabel("Entidad federativa : ");
		this.jlDomicilioParticularEntidadFederativa.setBounds(x, y + 320, 250, 20);
		this.jtDomicilioParticularEntidadFederativa = new JTextField();
		this.jtDomicilioParticularEntidadFederativa.setBounds(x + 300, y + 320, 200, 20);

		this.jlDomicilioParticularMunicipio = new JLabel("Municipio : ");
		this.jlDomicilioParticularMunicipio.setBounds(x, y + 350, 250, 20);
		this.jtDomicilioParticularMunicipio = new JTextField();
		this.jtDomicilioParticularMunicipio.setBounds(x + 300, y + 350, 200, 20);

		this.jlDomicilioParticularCodigoPostal = new JLabel("Codigo postal : ");
		this.jlDomicilioParticularCodigoPostal.setBounds(x, y + 380, 250, 20);
		this.jtDomicilioParticularCodigoPostal = new JTextField();
		this.jtDomicilioParticularCodigoPostal.setBounds(x + 300, y + 380, 200, 20);
		...
		this.jcDomicilioFiscal.setBounds(x + 200, y + 230, 150, 20);

		this.jcDomicilioParticular = new JCheckBox("Domicilio Particular");
		this.jcDomicilioParticular.setBounds(x + 350, y + 230, 160, 20);

		this.jbAgregarDomicilios = new JButton("Agregar Domicilio");
		this.jbAgregarDomicilios.setBounds(x + 520, y + 230, 150, 20);
		...
		this.jbAgregar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (jcDomicilioParticular.isSelected() == false && jcDomicilioFiscal.isSelected() == false) {
					...
					direc.setId(String.valueOf(id));
					direc.setNombre(jtNombre.getText());
					direc.setFechaNacimiento(jtFechaNacimiento.getText());
					direc.setSexo(jtSexo.getText());
					direc.setNss(jtNss.getText());
					direc.setSueldo(jtSueldo.getText());
					direc.setAccion(jtAccion.getText());
					DirectivoDao direcDao = new DirectivoDaoImpl();
					int filasAfectadas = direcDao.GrabarDirectivo(direc);
					if (filasAfectadas != 0) {
